 ...
 *
 * @author Baltan
 * @date 2018/2/17 15:18
 */
public class SameRatioExcel {
    ...
    public static void createExcel() {
        ...
        // 创建一个sheet
        HSSFSheet sheet = doc.createSheet("年度调整文件格式_统一比例单位公积金");
        // 设置表格列宽
        for (int i = 0; i < 8; i++) {
        ...
        // 创建第9个样式：11号、宋体、无边框、居中、自动换行、单元格为数值类型保留两位小数
        ...
        region = new CellRangeAddress(4, 4, 5, 7);
        sheet.addMergedRegion(region);
        setBorder(BorderStyle.THIN, region, sheet);
        ...
        try {
            fos = new FileOutputStream("/Users/Baltan/Desktop/年度调整文件格式_统一比例单位公积金.xls");
            doc.write(fos);
        } catch (IOException e) {
