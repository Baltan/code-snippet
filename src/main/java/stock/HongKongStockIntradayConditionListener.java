    ...
    private static void consecutiveRisingNotice(List<HongKongStockIntradayConditionResponse.Data.Stock> stocks) {
        stocks.parallelStream()
                .forEach(stock -> {
                    ...
                    if (risingFlag) {
                        String content = String.format("%s（%s）股价持续拉升，近%d次获取到股价为%s元，实时成交额为%s", stock.getF14(), stock.getF12(), CONSECUTIVE_RISING_STOCK_DEQUE_SIZE, prices, getAmount(stock.getF6(), Boolean.FALSE));
